# Part 12. 파이썬 심화
### ch 45~48 (Concurrency - 05. Coroutine - 1,2,3,4)
...
# yield : 메인루틴 <-> 서브 루틴, 여러 서브루틴을 실행 가능하게 해줌
...
print('EX1-1 -', c1, type(c1)) # 제너레이터 클래스
...
print('EX1-2 -', getgeneratorstate(c3))# 첫번째 yield문에서 대기

print(next(c3))

print('EX1-3 -', getgeneratorstate(c3))# 두번째 yield문에서 대기
...
print('EX3-4 -', exe_co.throw(SampleException)) # 예외에 걸려서 else문 실행 X
...
# gen1과 같은 기능이지만 좀 더 빠르게 사용 가능
def gen2():
    yield from 'AB' # from 뒤에 붙은 것을 알아서 제어함
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTkzNDM1NDZdfQ==
-->