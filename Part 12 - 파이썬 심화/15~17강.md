# Part 12. 파이썬 심화
### ch 15~17 (Data model - 05. Special Method(2) - 1,2,3)
...
# 파이썬에서 기본적으로 제공해주는  네이티브 메소드를 기반으로, 모든 것들은 객체로 표현 되기 때문에 우리가 필요로 하는 메소드를 오버로딩하여 원하는 흐름대로 코딩을 할 수 있도록 지원. 
# 매직메소드 기초 설명
# __  __ 형태
# 기본형
...
# 매직메소드 출력
# 매직 메소드를 통하여 하나의 메소드를 정의할 수 있음
print('EX2-1 -', s1 >= s2) # __ge__ 메소드 호출
print('EX2-2 -', s1 <= s2) # __le__ 메소드 호출
print('EX2-3 -', s1 - s2) # __sub__ 메소드 호출
print('EX2-4 -', s2 - s1)
print('EX2-5 - ', s1)
print('EX2-6 - ', s2)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQyODQxOTY2MV19
-->