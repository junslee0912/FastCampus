# Part 12. 파이썬 심화
### ch 36~40 (Object Reference - 04. Class Structure - 1,2,3,4,5)
...
class VectorP(object):
    ...
    def y(self, v):
        ...
        self.__y = v
# 내가 원하는대로 값이 훼손되지 않게 할 수 있음
# 안전한 클래스 설계

# 객체 선언
v5 = VectorP(20, 40)

# print('EX1-2 -', v5.__x, v5.__y) # 언더스코어 두번(__)은 해당 변수에 직접 접근 불가능하게 함

print('EX1-3 -',dir(v5), v5.__dict__) # property 때문에 x, y로 dir에 들어가있음
...
# print(v._x, v._y)
# v._y = 10 
# print(v._x, v._y) -> 이 경우 문제 발생, ValueError가 뜨지 않고, float형으로 바뀌지도 않고 그냥 출력됨
# 정보 은닉에 실패
...
# 클래스 자체를 많이 생성해야하는 경우 slot을 사용

# 성능 비교 (클래스 2개 선언)
class TestA(object):
    __slots__ = ('a',) # 이 클래스는 a라는 변수 밖에 가질 수 없음, 추가하려면 slot에 추가해야 함
...
# print('EX2-2 -', use_slot.__dict__) # __dict__를 가지고 있지 않음, 대신 set 형태 사용
print('EX2-3 -', no_slot)
...
print('EX3-1 -', min(timeit.repeat(repeat_outer(use_slot), number=1000))) # 시간반환함수
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg1MTU4Njk2NF19
-->